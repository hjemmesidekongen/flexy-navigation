...
if ( ! class_exists( 'FlexyNavigationWalker' ) ) {

    /**
     * Class FlexyNavigationWalker
     */
    class FlexyNavigationWalker extends Walker_Nav_Menu {
        ...
        public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            if ( $depth === 0 ) {
                ...
                $classes[] = 'flexy-navigation__item';

                if ( is_object( $args ) && $args->has_children ) {
                    $classes[] = 'flexy-navigation__item--dropdown';
                }

                //If we are on the current page, add the active class to that menu item.
                $classes[] = ( $item->current ) ? 'flexy-navigation__item--active' : '';

                // Active trail
                if ($ancestor_of_current) {
                    $classes[] = 'flexy-navigation__item--active-trail';
                }
            }
            ...
            // Dropdown
                ...
                //Add class and attribute to LI element that contains a submenu UL.
                $classes[] = 'flexy-navigation__item__dropdown-menu__item';

                if ( is_object( $args ) && $args->has_children ) {
                    $classes[] = 'flexy-navigation__item__dropdown-menu__item--dropdown';
                }

                //If we are on the current page, add the active class to that menu item.
                $classes[] = ( $item->current ) ? 'flexy-navigation__item__dropdown-menu__item--active' : '';

                // Active trail
                if ($ancestor_of_current) {
                    $classes[] = 'flexy-navigation__item__dropdown-menu__item--active-trail';
                }
            }
        ...
        public function start_lvl( &$output, $depth = 0, $args = array() ) {
            ...
            $output .= "\n$indent<ul class=\"flexy-navigation__item__dropdown-menu\">\n";
        }
    }
}
